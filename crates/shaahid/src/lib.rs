...
//! Shaahid workspace so a consumer can depend on one crate:
//!
...
//! deed into a ledger is the consumer's. See the crate-level docs of [`shaahid_contract`]
...
//! ledger) is the consumer's, never the core's (run `cargo test` to see it execute):
//!
//! ```
//! use shaahid::{Attestation, Deed, Fingerprint, witness};
//!
//! // The consumer's own witnessed ledger. The core never owns, mutates, or persists it.
...
//! ledger.push(first); // admission is the consumer's choice, not the core's
//!
