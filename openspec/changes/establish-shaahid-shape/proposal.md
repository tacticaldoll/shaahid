...
placeholders. It needs its initial project shape: the vision, the witness domain
language, the architectural axioms, executable governance, and a compiling crate
skeleton — enough that the adjudication core can be built from the repo alone in later
spec-driven changes, without losing the design knowledge that motivated it.

The core implementation is deliberately **not** included: the design still has open
questions (the `Attestation` shape, `Fingerprint` ownership, the `Contradiction`
taxonomy, the unenforceable purity invariant — see `BACKLOG.md`), and freezing them
into code now would violate least-commitment. This change establishes the shape and
the contract; implementation follows spec-driven.
...
- Replace the starter placeholders: `PROJECT.md` (vision, positioning, non-goals),
  ...
  `README.md`, and `docs/domain-language.md` (the witness vocabulary).
- Choose the crate layout and add two compiling crates: `shaahid-contract` (the
  isolated adjudication core, with the `Seal`, `Fingerprint`, and `Attestation`
  anchors and the sans-I/O axioms) and `shaahid-governance` (the Tianheng
...
- `adjudication-contract`: the core adjudication contract — the witness vocabulary,
  create-or-attach, the semantic bill of purity (no semantic judgment in the core),
  domain-supplied opaque `Seal`s, mechanical `Fingerprint`s, structural
  `Contradiction` detection, sans-I/O purity, and dependency isolation.
...
- New crates `shaahid-contract` and `shaahid-governance`; new `deny.toml`, CI, and the
